...
👷 Persona 1: O Operador de Campo Nome: Carlos Eduardo, 34 anos.
...
🦺 Persona 2: O Supervisor de Segurança (SST) Nome: Mariana Souza, 29 anos.
...
RF009 - Autenticação e Controle de Acesso: O sistema deve exigir identificação do supervisor (matrícula e senha) antes de liberar o acesso ao Dashboard Operacional (RF006), ao histórico de eventos (RF007) e aos relatórios de conformidade (RF008), garantindo que apenas usuários autorizados visualizem dados sensíveis de colaboradores e encerrando a sessão de forma explícita ao final do uso. (Requisito incorporado na Sprint 3, a partir da implementação da tela de login e logout no protótipo.)
